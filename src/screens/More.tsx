...
const More = ({
      ...
      items: [
        ...
        { icon: History,  label: 'Order History',  status: '',       color: 'text-blue-500', action: () => setActiveTab('orders') },
        {
          ...
          status:
            ...
            'Pending',
          color:
            user?.kyc_status === 'approved' ? 'text-emerald-500' :
            user?.kyc_status === 'rejected' ? 'text-rose-500' :
            'text-amber-500',
      ...
      items: [
        {
          ...
          status: '',
          color: 'text-zinc-600',
          action: () => { setComplianceType('SEBI Disclaimer'); setActiveTab('compliance'); }
        },
        {
          icon: AlertTriangle,
          ...
          status: '',
          color: 'text-zinc-600',
          action: () => { setComplianceType('Risk Disclosure'); setActiveTab('compliance'); }
        },
        {
          icon: FileText,
          ...
          status: '',
          color: 'text-zinc-600',
          action: () => { setComplianceType('Terms & Conditions'); setActiveTab('compliance'); }
        },
      ]
    }
  ];
  ...
  const sections = user?.role === 'admin'
    ...
    : allSections.filter(s =>
        ['Account', 'Subscription', 'Security & App', 'Compliance'].includes(s.title)
      );
      ...
      <div className="space-y-8 pb-24">
        ...
        {user && (
          <ProfileCard
            ...
            onStartKyc={() => setActiveTab('onboarding')}
          />
        )}
        ...
        <div className="text-center opacity-30 pb-10">
          <p className="text-[9px] font-bold text-zinc-500 uppercase tracking-[0.2em]">
            Aapa Capital v1.0.0 • SEBI INZ000123456
          </p>
        </div>
      </div>
