...
import {
  ArrowLeft, TrendingUp, ArrowUpRight, ArrowDownRight, Layers,
  ChevronDown, Zap, MousePointer2, Activity, BarChart3, Target,
  ArrowRightLeft, XCircle, ShieldCheck, AlertCircle, Lock, BarChart2
} from 'lucide-react';
    ...
    >
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] font-medium opacity-80 mt-1 leading-relaxed">{config.desc}</p>
        </div>
      </div>
      <button
...
const FOTradingCenter = ({
  ...
  const foEnabled      = Array.isArray(user?.segments_enabled) &&
    ...
    return (
      <div className="min-h-screen pb-24">
        ...
        <div className="mx-4 mt-6 rounded-2xl bg-zinc-900/30 border border-zinc-800/30 p-5 space-y-4">
          ...
          {[
            { icon: BarChart3,  color: 'text-purple-400', label: 'Live Option Chain — NIFTY, BANKNIFTY, SENSEX' },
            { icon: Zap,        color: 'text-amber-400',  label: 'Scalper Mode for lightning-fast order placement'  },
            { icon: Target,     color: 'text-blue-400',   label: 'GTT orders, Trailing SL &amp; multi-leg strategies' },
            { icon: Activity,   color: 'text-emerald-400',label: 'Real-time position P&amp;L with margin tracking'   },
            { icon: ShieldCheck,color: 'text-rose-400',   label: 'Risk management tools: SL/Target, hedge alerts'    },
        ...
        <div className="mx-4 mt-4 rounded-2xl bg-zinc-900/30 border border-zinc-800/30 p-5 space-y-3">
          ...
          {[
            ...
            { step: 2, label: 'Connect Upstox broker account',              done: !!user?.is_uptox_connected },
            { step: 3, label: 'Take risk questionnaire',                    done: hasRiskProfile  },
            { step: 4, label: 'Score Aggressive on risk profile',           done: foEnabled       },
          ].map(({ step, label, done }) => (
  ...
  // ── Normal FO screen (existing code, unchanged) ───────────────────────────
  return (
    <div className="space-y-6 pb-24">
      ...
      <AnimatePresence mode="wait">
        {activeChart && (
          <FullChartModal key="full-chart-modal" instrument={activeChart} onClose={() => setActiveChart(null)} />
        )}
